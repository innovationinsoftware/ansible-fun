...
On the Ansible Controller, create a directory `roles`:
...
1. Copy `web.yml` to the lab directory:
   ...
   ```bash
   cp /home/ansible/automation-dev/labs/roles/resources/web.yml ~/roles/
   ```
...
1. Log in to `node1`:
   ...
   You should see a new MOTD, so we know that play worked.
